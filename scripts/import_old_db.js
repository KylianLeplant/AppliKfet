...
const seededDepts = readSeedJson("category-depts.json");
const seededYears = readSeedJson("category-years.json");
const seededCategorySingles = readSeedJson("category-singles.json");
const seededProductCategories = readSeedJson("product-categories.json");
const seededProducts = readSeedJson("products.json");
// Ensure tables exist
...
async function runMigration() {
    ...
    const getCategoryId = (promo) => {
        if (!promo)
            return null;
        const p = promo.trim();
        // Rules
        if (p === "DI3")
            return catMap.get("DI 3A") ?? null;
        if (p === "DI4")
            return catMap.get("DI 4A") ?? null;
        if (p === "DI5")
            return catMap.get("DI 5A") ?? null;
        if (p === "DII")
            return catMap.get("ISIE sans classe") ?? null;
        if (p === "DMS")
            return catMap.get("DMS sans classe") ?? null;
        if (p === "DA")
            return catMap.get("DA sans classe") ?? null;
        if (p === "Peip")
            return catMap.get("Peip sans classe") ?? null;
        if (p === "Save")
            return catMap.get("Save") ?? null;
        if (p === "Autre")
            return catMap.get("Autre") ?? null;
        if (p === "DI6")
            return catMap.get("Save") ?? null;
        if (p === "KFtier")
            return catMap.get("KFetier") ?? null;
        return null;
    };
    const newCustomers = [];
...
async function seedProducts() {
    ...
    try {
        ...
        if (existingProducts.length === 0) {
            ...
            const allProductCats = await db.select().from(productsCategories);
            const categoryIdByName = new Map(allProductCats.map((cat) => [cat.name, cat.id]));
